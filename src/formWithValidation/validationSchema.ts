...
export const validationSchema = yup.object().shape<FormState>({
  ...
  age: yup.number().required('You must have an age').min(0, 'No one is less that zero years!').max(100, 'You are just too old'),
  zip: yup.string().required('You must enter a zip code').matches(/^\d\d\d\d$/,'This does not look like a valid zip code'),
